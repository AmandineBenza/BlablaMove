...
public class TestBilling {
	...
	public void setup() {
		...
		floatVolume = 30.7;
		nearDate = 1;
		offerA = "{\"ownerID\" : \"user@mail.com\", \"weight\": \"" + String.valueOf(weight) + "\"," + "\"volume\":\""
				+ String.valueOf(volume) + "\", \"date\":\"" + String.valueOf(farDate) + "\"}";
	...
	public void basicCalculTest() {
		...
		calculator.calcul_price_base(data.get("weight").getAsDouble(), distance, data.get("volume").getAsDouble(),
	...
	public void floatCalculTest() {
		...
		calculator.calcul_price_base(data.get("weight").getAsDouble(), floatDistance, data.get("volume").getAsDouble(),
				data.get("date").getAsInt());
		assertEquals(expected, calculator.getUserPoints(), 0.1);
		calculator.finalConfirmation(calculator.getUserPoints());
		assertEquals((expected / 100) * 90.0, calculator.getUserPoints(), 0.1);
